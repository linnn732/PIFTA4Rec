...
parser.add_argument('--test_every_epoch', type=int, default=1, help='max_basket_num')
...
parser.add_argument('--G1_flag', type=int, default=-1, help='train_type :  with G1 1 / with no G1 -1 / from no G1 to G1 0')
...
parser.add_argument('--test_type', type=int, default=2200, help='')  # TaFeng:12200, Dunnhumby: 2200
...
parser.add_argument('--dataset', type=str, default='TaFeng', help='dataset name') # 資料集名稱 TaFeng, Dunnhumby, Instacart
parser.add_argument('--num_product', type=int, default=12085 , help='n_items TaFeng:9963 Instacart:8222 Delicious:6539') # 項目總數: TaFeng:15764, Dunnhumby:3977, Instacart:27095
parser.add_argument('--num_users', type=int, default= 13972, help='n_users TaFeng:16060 Instacart:6885 Delicious:1735')  # 用戶總數: TaFeng:10654, Dunnhumby:19132, Instacart:19485
parser.add_argument('--distrisample', type=int, default= 0, help='')

parser.add_argument('--output_dir', type=str, default='./result', help='')
parser.add_argument('--log_fire', type=str, default='test', help='basket_group_split')
parser.add_argument('--temp', type=float, default=1, help='')
parser.add_argument('--temp_min', type=float, default=0.3, help='')
parser.add_argument('--pretrain_epoch', type=int, default= 2, help='n_users TaFeng:16060 Instacart:6885 Delicious:1735')
parser.add_argument('--batch_size', type=int, default=128, help='batch_size') # 論文 batch size 128
parser.add_argument('--epoch', type=int, default=35, help='the number of epochs to train for')
parser.add_argument('--ANNEAL_RATE', type=float, default=10, help='ANNEAL_RATE') # 論文 10
parser.add_argument('--lr', type=float, default=0.0001, help='learning rate') # 論文 0.0001
...
parser.add_argument('--embedding_dim', type=int, default=64,help='hidden sise') # 論文 64 dim
parser.add_argument('--alternative_train_epoch', type=int, default=1, help='max_basket_num') #default=5
...
parser.add_argument('--history', type=int, default=1, help='history') # 訓練時歷史項目需不需要排除(1:保留、0:排除)
parser.add_argument('--temp_learn', type=int, default=0, help='temp_learn')
parser.add_argument('--before_epoch', type=int, default=0, help='basket_group_split')
parser.add_argument('--output_number', type=int, default=1, help='output_number') # 記錄第幾次訓練用
...
class Config(object):
    def __init__(self):
        ...
        self.MODEL_DIR = './runs'
        self.input_dir = 'dataset/{}'.format(args.dataset)+'.txt'
        ...
        self.output_number = args.output_number  
